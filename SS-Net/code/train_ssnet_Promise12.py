...
def computer_complexity_with_thop(model,input_size):
    dummy_input = torch.randn(1,*input_size).cuda()
    flops,params = profile(model,inputs=(dummy_input,),verbose=False)
    params_m = params / 1e6
    gflops = flops/1e9
    flops_formatted,params_formatted = clever_format([flops,params],"%.2f")
    print('\n==============thop complexity  analysis=======')
    print(f"Params:{params_formatted}(={params_m:.2f} M)")
    print(f"gflops:{gflops:.2f}(original flops:{flops_formatted})")
    return params_m,gflops
def monitor_gpu_memory(iter_num, model, input_tensor):
    """监控训练时的GPU显存占用（Train RAM）"""
    torch.cuda.synchronize()
    peak_mem_mb = torch.cuda.max_memory_allocated() / (1024**2)
    current_mem_mb = torch.cuda.memory_allocated() / (1024**2)
    print("\n===== GPU Memory Usage =====")
    print(f"Iter {iter_num} - Current Train RAM: {current_mem_mb:.0f} MB")
    print(f"Iter {iter_num} - Peak Train RAM: {peak_mem_mb:.0f} MB")
...
def train(args, snapshot_path):
    ...
    for epoch_idx in iterator:
        ...
        print(f"Epoch{epoch_idx+1} spend:{epoch_time:.2f} s")
        if iter_num >= max_iterations:
            iterator.close()
            break
    # writer.close()
