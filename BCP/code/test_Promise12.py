...
parser.add_argument('--stage_name', type=str, default='self_train_JS', help='self or pre')
...
#         label = h5f['label'][:]
#         prediction = np.zeros_like(label)
#         cam_maps = np.zeros_like(image, dtype=np.float32)  # 存储Grad-CAM结果
#
#         # 注册钩子
#         target_layer = net.encoder.down4.maxpool_conv[1].conv_conv[4]  # 最后一层编码器的第二个卷积
#         activations = {}
#         gradients = {}
#
#         def forward_hook(module, input, output):
...
#                 # 原始图像尺寸
#                 slice = image[ind, :, :]
#                 orig_h, orig_w = slice.shape[0], slice.shape[1]  # 记录原始尺寸
#
#                 # 预处理
#                 slice_resized = zoom(slice, (256 / orig_h, 256 / orig_w), order=0)
#                 input_tensor = torch.from_numpy(slice_resized).unsqueeze(0).unsqueeze(0).float().cuda()
#                 input_tensor.requires_grad = True
#
#                 # 前向传播
...
#                 # 反向传播
#                 net.zero_grad()
#                 out_main[:, 1].mean().backward()  # 使用前景类别（假设通道1）
#
#                 # 生成热力图
#                 acts = activations['layer'].squeeze(0).cpu().numpy()  # [C, H, W]
#                 grads = gradients['layer'].squeeze(0).cpu().numpy()
...
#                 scale_h = orig_h / cam_h
#                 scale_w = orig_w / cam_w
#                 cam = zoom(cam, (scale_h, scale_w), order=1)  # 线性插值
#
#                 # 存储结果
#                 cam_maps[ind] = cam
#
...
#                 [image, prediction, label, cam_maps],
#                 ["_img", "_pred", "_gt", "_cam"]
#         ):
#             itk_image = sitk.GetImageFromArray(data.astype(np.float32))
#             itk_image.SetSpacing(spacing)
#             sitk.WriteImage(itk_image, f"{test_save_path}/{case}{suffix}.nii.gz")
#
#         # 指标计算
#         if np.sum(prediction == 1) == 0:
#             return (0, 0, 0, 0)
...
def Inference(FLAGS):
    ...
    snapshot_path = "./model/BCP_LPP/Promise12/Promise12_{}_{}_labeled/{}".format(FLAGS.exp, FLAGS.labelnum, FLAGS.stage_name)
    test_save_path = "./model/BCP_LPP/Promise12/Promise12_{}_{}_labeled/{}_predictions_CAM_Promise12/".format(FLAGS.exp, FLAGS.labelnum, FLAGS.model)
    ...
    save_model_path = os.path.join(snapshot_path, "unet_best_model2.pth")
